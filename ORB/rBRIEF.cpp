...
void rBRIEF(cv::Mat **imgPyr, vector<Feature> keyPlist, vector<FingerPrint> FP){
    ...
    for(i=0;i<nkeyP;i++){
        const Feature& keyP = keyPlist[i];
        cv::Mat* img = imgPyr[keyP.level];
        float angle = keyP.orien,//already radian scale
              a = (float)cos(angle),
              b = (float)sin(angle);
        ...
        for(j=0;j<256;j++){
            ...
            _x2 = (float)xy_index[4*j+2];   _y2 = (float)xy_index[4*j+3]  ;
